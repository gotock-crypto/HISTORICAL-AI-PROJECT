...
class HistoryEngine:
    ...
    def prepare_daily_batch(self, force=False):
        ...
        try:
            ...
            for item in selected:
                ...
                try:
                    candidates = self.search_daily_media(p)
                    for pos, candidate in enumerate(candidates[:int(self.setting("daily", "images_per_event", 4))], 1):
                        self.db.add_daily_media(row["id"], candidate, pos)
                    if not candidates:
                        self.db.set_daily_event_status(row["id"], "ready", "no media candidates prepared")
                except Exception as exc:
                    log.warning("DAILY MEDIA PREP FAILED | event=%s | %s", row["title"], exc)
                    self.db.set_daily_event_status(row["id"], "ready", f"media preparation: {exc}")
    ...
    def next_daily_event(self):
        ...
        rows = self.db.ready_daily_events(batch_date)
        for row in rows:
            if self.db.topic_used(row["topic_key"]):
                self.db.set_daily_event_status(row["id"], "posted", "already used in history")
                continue
            self.db.set_daily_event_status(row["id"], "processing", "reserved for draft")
            return self._daily_event_payload(row)
        return None
    ...
    def search_daily_media(self, p):
        ...
        if len(proper) >= 2:
            queries.append(
                f'"{" ".join(proper[:3])}" {year}'
            )

        if len(anchors) >= 2:
            queries.append(
                f'"{anchors[0]}" "{anchors[1]}" {year}'
            )
        ...
        # 5. Individual concrete anchors as fallback.
        for anchor in anchors[:8]:
            queries.append(f'"{anchor}" {year}')

        # 6. Generic historical fallback.
        if title:
            queries.append(f'"{title}" historical photograph {year}')
...
SOURCE: {x.get('source_url', '')}"""
    ...
    def gigachat(self, ctx):
        prompt_name = "story_writer.txt" if "FACT_CONTEXT:" in ctx else "archive_writer.txt"
        ...
        for attempt in range(1, 5):
            try:
                ...
                if r.status_code in (429, 500, 502, 503, 504):
                    raise requests.HTTPError(f"{r.status_code} temporary response")
                r.raise_for_status()
                ...
                last = e
                log.warning("GIGACHAT RETRY | %s/4 | %s", attempt, e)
                if attempt < 4:
    ...
    def _history_primary_subject_score(self, p, x):
        ...
        # A concrete anchor must be present in semantic metadata.
        # A filename hit alone is never sufficient.
    ...
    def validate_text(self, text, x, p=None):
        text = str(text or "").strip().replace(" .", ".")
        lo = int(self.setting("content", "min_chars", 700))
        hi = int(self.setting("content", "max_chars", 1300))
        if not text:
        ...
        if len(text) < lo and x.get("source_name") != "text-only fallback":
    ...
    def generate_draft(self):
        """Build a draft from the next persisted daily event.

        Event discovery and media discovery are already completed by the daily
        preparation job. This stage only validates/downloads the best available
        image and asks GigaChat to write the post.
        """
        p = self.next_daily_event()
        if not p:
            raise RuntimeError("На сегодня больше нет готовых исторических событий")

        media_rows = self.db.daily_event_media(p["daily_event_id"])
        last_error = None
        for idx, row in enumerate(media_rows, 1):
            x = dict(row)
            try:
                x = self.download_media(x)
                reject, reason = self._image_visual_reject(x)
                if reject:
                    raise RuntimeError(reason)
                subject = self._history_primary_subject_score(p, x)
                if subject < float(self.setting("media", "min_primary_subject_score", 4)):
                    raise RuntimeError(f"weak_subject={subject:.1f}")
                visual = self._image_visual_score(x)
                x["primary_subject_score"] = subject
                x["visual_score"] = visual
                x["final_image_score"] = float(row.get("score",0)) + subject + visual
                x["daily_event_id"] = p["daily_event_id"]
                # The first valid candidate is not necessarily the best: continue
                # through prepared candidates and retain the highest score.
                if "best" not in locals() or x["final_image_score"] > best["final_image_score"]:
                    if "best" in locals() and best.get("media_path"):
                        Path(best["media_path"]).unlink(missing_ok=True)
                    best = x
                else:
                    Path(x["media_path"]).unlink(missing_ok=True)
            except Exception as exc:
                last_error = exc
                self.db.set_daily_event_status(p["daily_event_id"], "processing", f"image {idx}: {exc}")
                log.info("DAILY IMAGE REJECTED | event=%s | candidate=%s | %s", p["title"], idx, exc)
                if x.get("media_path"):
                    Path(x["media_path"]).unlink(missing_ok=True)

        if "best" not in locals():
            self.db.set_daily_event_status(p["daily_event_id"], "skipped", f"all images unavailable: {last_error}")
            raise RuntimeError(f"Все изображения события недоступны: {last_error}")

        try:
            raw = self.gigachat(self.context(p, best))
            ok, reason = self.validate_text(raw, best, p)
            if not ok:
                # One content retry is enough; never create an API storm.
                raw = self.gigachat(self.context(p, best) + "\nСделай текст немного короче и строго опирайся на факты события.")
                ok, reason = self.validate_text(raw, best, p)
            if not ok:
                raise RuntimeError(reason)
            best["topic_key"] = p["topic_key"]
            best["topic"] = p["title"]
            log.info("DRAFT READY | event_id=%s | %s", p["daily_event_id"], p["title"])
            return p["title"], best, raw
        except Exception as exc:
            self.db.set_daily_event_status(p["daily_event_id"], "ready", f"draft generation: {exc}")
            if best.get("media_path"):
                Path(best["media_path"]).unlink(missing_ok=True)
            raise
